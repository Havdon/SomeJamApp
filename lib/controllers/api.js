...
exports.addAttendEvent = function(req, res) {
	...
	if(eventId === undefined) {
		return res.json({ success: false, error: "Event id was not found." });
	}
	...
	return User.findByIdAndUpdate(id, {$push: {events: eventId}}, {upsert: true}, function (err) {
			    if (err) return res.json(500, {success: false, error: "Failed to push event into user." });
			    return Event.findByIdAndUpdate(eventId, {$push: {users: id}}, {upsert: true}, function (err) {
