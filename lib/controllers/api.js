...
/**
*	search for an interest using a string
*/
...
exports.addInterestToUser = function(req, res) {
	...
	if(interestId === undefined) {
		return res.json(JSON.stringify({ success: false, error: "Interest id was not found." }));
	}

	return User.findByIdAndUpdate(id, {$push: {interests: interestId}}, {safe: true, upsert: true}, function (err) {
			    if (err) return res.json(JSON.stringify({ success: false, error: "Failed to find user." }));
...
exports.addAttendEvent = function(req, res) {
	...
	return User.findByIdAndUpdate(id, {$push: {events: eventId}}, {safe: true, upsert: true}, function (err) {
			    if (err) return res.json(JSON.stringify({ success: false, error: "Failed to push event into user." }));
			    return Event.findByIdAndUpdate(eventId, {$push: {users: id}}, {safe: true, upsert: true}, function (err) {
				    if (err) return res.json(JSON.stringify({ success: false, error: "Failed push user into event." }));
				    return res.json(JSON.stringify({success: true}));
				  });
